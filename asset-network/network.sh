...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Org7 Identities ######################"
    echo "##########################################################"

    createOrg7

    echo "##########################################################"
    echo "############ Create Org8 Identities ######################"
    echo "##########################################################"
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile AssetOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
