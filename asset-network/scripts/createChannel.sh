...
createChannelTx() {
	...
	set -x
	configtxgen -profile AssetChannel -outputCreateChannelTx ./channel-artifacts/${CHANNEL_NAME}.tx -channelID $CHANNEL_NAME
	res=$?
	set +x
	if [ $res -ne 0 ]; then
...
createAncorPeerTx() {
	...
	for orgmsp in Org1MSP Org2MSP Org3MSP Org4MSP Org5MSP Org6MSP Org7MSP Org8MSP; do

	echo "#######    Generating anchor peer update transaction for ${orgmsp}  ##########"
	set -x
	configtxgen -profile AssetChannel -outputAnchorPeersUpdate ./channel-artifacts/${orgmsp}anchors.tx -channelID $CHANNEL_NAME -asOrg ${orgmsp}
	res=$?
	set +x
	if [ $res -ne 0 ]; then
...
joinChannel 6
echo "Join Org7 peers to the channel..."
joinChannel 7
echo "Join Org8 peers to the channel..."
joinChannel 8
